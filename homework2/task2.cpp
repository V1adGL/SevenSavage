...
int main(int argc, char** argv)
{
    if (argc != 2) { cout << "There aren't any arguments or there are more arguments than we expected."; }
        ...
        while (h > 0) {
            ...
            // проверяем что оно упало
            if (h <= 0) {
                answer = i;
                break;
            }
            // проверяем что оно перелетело
            if (h > hk[i] && i == xk.size() - 1) {
                answer = xk.size();
                break;
            }
            // иначе: столкнулось, значит меняем скорость по иксу
            else { vx = -vx; }
            do {
                i--;
                t = (xk[i] - xk[i_prev]) / vx;
            ...
            // если тело упало то выводим ответ
            if (h <= 0) {
                answer = i_prev + 1;
                break;
            }
            // тело после столкновения упало в нулевую зону
            if (h > hk[i] && i == 0) {
                answer = 0;
                break;
            }
